...
pts <- lapply(1:nrow(nps), function(x) {
  point    <- spsample(nps[x, ], type = "random", n = 100)
  point    <- as(point, "SpatialPointsDataFrame")
  point@data <- data.frame(SourceArea = rep(x, length(point)))
  return(point)
})
...
sims <- tibble(ID = 1:length(pts), SourceArea = pts$SourceArea)
